...
function attach_webcam(deviceId,index){
  ...
  function scanner_snap() {
    ...
    document.querySelector('#scanner_result_container').innerHTML = '<img id= "scanner_webcam" src = "'+ dataUrl_1 +'">';
...
$(snap_button).on('click',function(){

  let faculty_qr = $('#faculty').val();
  $.ajax({
    url:'selfieCapture.php',
    method:'POST',
    data:{webcam:1,faculty_qr:faculty_qr},
    success:function(){
...
.then(function(devices){
  ...
  let camera = 0; //index for used camera
    ...
    snap_button.addEventListener('click',function(){
      ...
      camera_autoCapture(`#results_container_${index}`,online_camera)
      .then((autoCapture)=>{
        index--
        camera--
        });
...
function camera_autoCapture(img_container,camera){
  ...
  return new Promise((resolve)=>{
   ...
   function autoCapture_camera(img_container){
    ...
    document.querySelector(img_container).innerHTML = '<img id= "webcam" src = "'+ dataUrl +'">';
